...
public class Nodes {
	...
	public Node2 getNode2(Role r)
	{
		...
		{
			if(node2[i].getNodeSemantics().getPre().getTMSPrincipal().equals(r.getPre().getTMSPrincipal())
					&& node2[i].getNodeSemantics().getPost().getRoleName().equals(r.getPost().getRoleName()))
				return node2[i];
		}
		return null;
	}
